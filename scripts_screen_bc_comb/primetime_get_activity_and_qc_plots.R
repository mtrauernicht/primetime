...
# Handle list of annotated files: read from file if it's a path, otherwise parse as space-separated
if (file.exists(opt$list_of_annotated_files)) {
    # It's a file path, read the list from the file
    list_of_annotated_files <- trimws(readLines(opt$list_of_annotated_files))
    list_of_annotated_files <- list_of_annotated_files[list_of_annotated_files != ""]  # Remove empty lines
} else {
    # It's a space-separated or directly provided list
...
pDNA <- counts_df %>%
    ...
    rowwise() %>%
    mutate(pDNA = mean(c_across(-barcode), na.rm = TRUE)) %>%
    ungroup() %>%
    select(barcode, pDNA)
...
for (this_cdna_sample in cdna_sample) {
    ...
    for (this_pdna_sample in pdna_sample) {
        ...
        # Check if we have valid data (non-NA cases) before fitting
        ...
        if (sum(valid_pairs) >= 2) {
            ...
            # If not enough data, use slope=1 and intercept=0 as default
            message("Warning: Not enough valid negative control data for ", this_cdna_sample, " vs ", this_pdna_sample, ". Using default slope=1, intercept=0.")
            equation_df <- rbind(
                equation_df,
                data.frame(
                    cDNA_sample = this_cdna_sample,
