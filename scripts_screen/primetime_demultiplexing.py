...
def get_barcode_counts(fastq_r2,
    ...
    # Dictionary to hold open file handles for each sample
    out_writers = {}
    ...
    def process_batch(batch):
        ...
        for seq, read_id, r1_fastq in batch:
            ...
            local_sample_reads.setdefault(sample, []).append(r1_fastq)
        ...
        return (local_matched_valid, local_matched_invalid, local_mismatched,
                local_invalids, local_sample_reads, local_barcode_ids,
    ...
    def handle_result(result):
        ...
        (loc_valid, loc_invalid, loc_mismatched, loc_invalids,
         loc_sample_reads, loc_barcode_ids, loc_well_counts, loc_sample_counts) = result
        ...
        for sample, reads in loc_sample_reads.items():
            if plate_id:
                out_path = os.path.join(out_base, f"{plate_id}_{sample}.fastq.gz")
            else:
                out_path = os.path.join(out_base, f"{sample}.fastq.gz")
